...
are designed to be used over TCP/IP connection, such as http://mqtt.org[MQTT], 
...
serialised. Very similar to <<fields-fields>>.  
...
<<fields-fields>>, which immediately takes us to the direction of
reusing <<fields-fields, Field>> classes to handle these values:
[source, c++]
----
...
https://en.wikipedia.org/wiki/OSI_model[OSI Conceptual Model], where
